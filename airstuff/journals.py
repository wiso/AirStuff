    ...
    def wrap(f):
        ...
        def wrapped_f(*args, **kwargs):
            ...
            if tstop - tstart > max_time:
                logging.warning('function %s with arguments %s %s took much time: %f ms', f.__name__, args, kwargs, (tstop - tstart) * 1000)
            return result
        return wrapped_f
    return wrap
...
def get_redirect_doi(doi):
    ...
    if not redirect:
        logging.warning('cannot get redirect for doi %s', url_doi)
        return None
    return redirect


def get_pdf_url(doi):
    logging.debug('getting redirect for doi %s', doi)
    url = get_redirect_doi(doi)
    if not url:
        logging.warning('cannot resolve doi %s', doi)
        ...
        logging.debug('tentative url: %s from doi: %s', tentative_url, doi)
    ...
    logging.debug('redirect from doi %s is %s', doi, url)
    if hostname == 'link.springer.com':
        ...
        logging.error('not able to get pdf for %s from %s', doi, url)
        return None
...
def get_pdf_url_springler(doi, url):
    ...
    if r:
        return r
    logging.warning('problem to find pdf link for %s on springler, try another method', doi)
...
def get_pdf_url_springler_tight(url):   
    ...
    if len(els) != 1:
        logging.error('more than one download link on %s', url)
        return None
    elif not els:
        logging.error('no download link on %s', url)
        return None
